...
--[[
    ...
    Protocol Structure:
    -------------------
    All messages start with a 4-byte fixed header:
      - Version        : 1 byte
      - Message Type   : 1 byte
      - Payload Length : 2 bytes

    Information Elements (IEs) follow the header:
      - Each IE is TLV-encoded:
          * Type (1 byte)
          * Length (2 bytes)
    ...
    Supported IEs:
    --------------
      -- General Setup and Control IEs:
         * 0x01: YSM ID (uint32)
         * 0x02: Location (string)
         * 0x03: Capabilities (uint8 bitfield)
         * 0x04: Scan Bands (list of uint16 MHz entries)
         * 0x05: FFT Size (uint16)
         * 0x06: Max Power (int16 dBm)
         * 0x11: Status Code (uint8)
         * 0x12: Response Message (string)
         * 0x13: Assigned Frequency (uint16 MHz)
         * 0x14: Assigned Bandwidth (uint16 MHz)
         * 0x15: Allocated Power (int8 dBm)

      -- Radio Resource Descriptor IE:
         * 0x20: Radio Resource Info (4 bytes total)
             - Resource Type (uint8)
             - Frequency (uint8)
             - Band (uint8)
             - Power (uint8)
      ...
      -- YSMReport IEs:
         * 0x40: Timestamp (string)
         * 0x41: Start Frequency (uint32 MHz)
         * 0x42: End Frequency (uint32 MHz)
         * 0x43: Occupancy (%) (uint8)
         * 0x44: Number of Measurements (uint16)
         * 0x45: Decision (string)

    Features:
    ---------
    - Adds protocol and info columns in packet view.
    - Validates assigned frequency against scanned bands.
    - Handles malformed or unknown IEs gracefully.
    - Highlights expert info for warnings or errors.

    Usage:
    ------
    1. Copy this Lua file into your Wireshark plugins directory.
    2. Start (or restart) Wireshark.
    3. Ensure that UDP packets on port 9090 are captured or loaded.

    License:
    --------
    This script is released under MIT License. Modify and use freely in academic or research tools.
